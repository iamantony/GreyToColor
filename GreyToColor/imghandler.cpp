...
void ImgHandler::setImg(QImage t_img, int t_type)
{
...
QImage ImgHandler::getImg(int t_type)
{
	return m_imgMass[t_type];
}

void ImgHandler::getLumImg(QImage *t_img)
{
	if (t_img == NULL)
		return;
...
QImage ImgHandler::startImgColorizationWSimple()
{
...
// Colorize our gray image and compare original coloured TARGET image with a RESULT
QImage ImgHandler::startImgColorizationWNeighbor()
{
...
void ImgHandler::imageComparison(QImage &t_targ, QImage &t_coloured)
{
...
void ImgHandler::getGrayImg(QString t_imgPath)
{
...
//
void ImgHandler::getColorImg(QString t_imgPath)
{
	QImage *img = new QImage(t_imgPath);
...
void ImgHandler::setImg2Label(QImage *t_img, int t_type)
{
	...
	switch(t_type)
	{
		...
		default:
			qDebug() << "In ImgHandler::setImg2Label(): unknown image type.";
	}
}
